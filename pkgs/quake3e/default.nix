...
, curl, libGL, vulkan-loader, libX11, libXxf86dga, alsa-lib, libXrandr, libXxf86vm, libXext, SDL2, glibc
}:
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ curl libGL vulkan-loader libX11 libXxf86dga alsa-lib libXrandr libXxf86vm libXext SDL2 glibc ];
  env.NIX_CFLAGS_COMPILE = "-I${SDL2.dev}/include/SDL2";
  enableParallelBuilding = true;

  postPatch = ''
    sed -i -e 's|dlopen( *dllname *[,)]|dlopen( "${libGL}/lib/libGL.so",|' \
      code/unix/linux_qgl.c
    sed -i 's|Sys_LoadLibrary( *dllname * )|Sys_LoadLibrary( "${vulkan-loader}/lib/libvulkan.so" )|' \
      code/unix/linux_qvk.c
